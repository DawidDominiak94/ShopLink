    ...
    <div>
      <ul>
        <li v-for="element of syncedUsersData" :key="element.id" class="flex w-full p-4 justify-items-center items-center">
          ...
          <div class="w-10 h-10 text-red-500 cursor-pointer flex" @click="deleteConnection(element)">
...
<script lang="ts" setup>
  ...
  async function deleteConnection( connection : SyncedUsers )
  {
    const isDeleted = await useSupabaseRepo().deleteConnection( connection.id );
    if( isDeleted )
    {
